        ...
        <div class="row">
          <div class="col-12 settingsPage">
            <nav class="overflow-auto nav-withScroll">
              <div class="nav flex-nowrap text-center d-inline-flex d-sm-flex nav-tabs bg-white shadow pt-1 pe-2 ps-2"
                id="nav-tab" role="tablist">
                <router-link :to="{ path: '/admin/settings', query: { tab: 'home' } }" class="nav-link"
                  @click.prevent="setActive('home')" :class="{ active: isActive('home') }">
                  <span class="p-1 span"> Profile </span>
                </router-link>
                <router-link class="nav-link" :to="{ path: '/admin/settings', query: { tab: 'team' } }"
                  @click.prevent="setActive('team')" :class="{ ' active': isActive('team') }">
                  <span class="p-1 span">Team</span>
                </router-link>
                <router-link :to="{ path: '/admin/settings', query: { tab: 'company' } }" class="nav-link"
                  @click.prevent="setActive('company')" :class="{ ' active': isActive('company') }">
                  <span class="p-1 span">Company</span>
                </router-link>
                <router-link :to="{ path: '/admin/settings', query: { tab: 'billing' } }" class="nav-link"
                  @click.prevent="setActive('billing')" :class="{ ' active': isActive('billing') }">
                  <span class="p-1 span"> Billings & Payment</span>
                </router-link>
                <router-link class="nav-link" :to="{ path: '/admin/settings', query: { tab: 'security' } }"
                  @click.prevent="setActive('security')" :class="{ ' active': isActive('security') }">
            ...
            <div class="tab-content mt-1 mt-lg-3" id="nav-tabContent">
              ...
              <div class="tab-pane fade" :class="{ 'active show': isActive('billing') }" id="billing">
                <Billings />
              </div>
