<template>
  <div class="grid grid__2 mx-auto" :style="{ width: size + 'px' }">
    <div class="cut">
      <vue-cropper ref="cropper" :img="data" :output-size="option.size" :output-type="option.outputType" :info="true"
        :full="option.full" :fixed="fixed" :fixed-number="fixedNumber" :can-move="option.canMove"
        ...
        :max-img-size="option.max" @crop-moving="cropMoving" mode="contain"></vue-cropper>
    </div>
    <div class="show-preview shadow" :style="{
      width: previews.w + 'px',
  ...
  <div>
    <button @click="finish()" class="btn btn-primary d-block ms-auto mt-2">
      {{ btnText }}
    </button>
  </div>
</template>
